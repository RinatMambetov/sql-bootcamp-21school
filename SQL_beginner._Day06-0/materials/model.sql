...
CREATE TABLE person_visits
    (
        ...
        CONSTRAINT uk_person_visits
            UNIQUE (person_id, pizzeria_id, visit_date),
...
CREATE TABLE menu
    (
        ...
        CONSTRAINT fk_menu_pizzeria_id
            FOREIGN KEY (pizzeria_id) REFERENCES pizzeria (id)
    );
...
CREATE OR REPLACE VIEW v_cheaper_pz
AS
SELECT name
  FROM pizzeria pz
           JOIN menu m
...
CREATE OR REPLACE VIEW v_dmitriy_new_visit
AS
SELECT id
  FROM pizzeria
...
DROP VIEW IF EXISTS v_dmitriy_new_visit;
DROP VIEW IF EXISTS v_cheaper_pz;
